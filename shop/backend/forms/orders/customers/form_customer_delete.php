...
<div class="bg-[#1A1A1A] p-10 rounded-2xl shadow-2xl text-center max-w-md w-full mx-auto border border-[#737373]/30">
    <?php if ($customerEliminado === true): ?>
        ...
        <p class="mb-8 text-texto">¿Seguro que quieres borrar el cliente con ID <span class="text-[#00C4CC] font-semibold"><?= $customer_id ?></span>?</p>
        <form method="POST" class="flex justify-center gap-4">
            <input type="hidden" name="product_id" value="<?= $customer_id ?>">
            <input type="submit" value="Aceptar" class="bg-[#FF4D00] text-texto px-6 py-2 rounded-xl font-bold hover:bg-[#00C4CC] hover:text-[#0D0D0D] cursor-pointer transition-all duration-200">
            <button type="button" onclick="window.history.back()" class="bg-[#737373] text-texto px-6 py-2 rounded-xl font-bold hover:bg-[#00C4CC] transition-all duration-200 cursor-pointer">Cancelar</button>
        </form>
    <?php else: ?>
        <h1 class="text-2xl font-bold mb-6 text-texto">No se pudo eliminar</h1>
