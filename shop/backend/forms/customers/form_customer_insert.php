        ...
        <div class="bg-[#1A1A1A] p-10 rounded-2xl shadow-2xl text-center max-w-md w-full border border-[#737373]/30">
            ...
            <?php if ($customerInsertado === true) { ?>
                ...
                <p class="mb-8 text-[#F5F5F5]">
                    El cliente <span class="text-[#00C4CC] font-semibold"><?= htmlspecialchars($_POST['customer_name']) ?>
                        <?= htmlspecialchars($_POST['customer_lastname']) ?></span> se ha insertado correctamente.
                </p>
            <?php } ?>
            ...
            <?php if ($customerInsertado === null) { ?>
                ...
                <form method="POST" class="flex flex-col items-center space-y-4">
                    <input type="text" name="customer_name" placeholder="Nombre" required
                        class="w-64 p-2 rounded-xl border border-[#737373] text-[#0D0D0D] focus:outline-none focus:ring-2 focus:ring-[#00C4CC]">
                    <input type="text" name="customer_lastname" placeholder="Apellido" required
                        class="w-64 p-2 rounded-xl border border-[#737373] text-[#0D0D0D] focus:outline-none focus:ring-2 focus:ring-[#00C4CC]">
                    ...
                    <input type="submit" value="Insertar"
                        class="w-64 bg-[#FF4D00] text-[#F5F5F5] py-2 rounded-xl font-bold cursor-pointer hover:bg-[#00C4CC] hover:text-[#0D0D0D] transition-all duration-200">
                </form>
            <?php } ?>

            <a href="../../index.php" class="block mt-6 text-sm text-[#737373] hover:text-[#00C4CC] transition">← Volver
                al inicio</a>
        </div>
    </div>
